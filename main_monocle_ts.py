...
parser.add_argument('--interv', type=int)
...
gn_fname = inpath + 'gene.txt'

gene_name = []

with open(gn_fname) as f:
    gene_name = f.readline().strip().split()

raw_data = np.load(inpath + args.data + '.npz')

# smooth with sliding winndow

width = args.width
data = np.zeros((raw_data.shape[0]-width, raw_data.shape[1]))

for t in range(raw_data.shape[0]-width):
    data[t, :] = np.mean(raw_data[t:t+width, :], axis=0)
...
for cur_n in range(0, trX.shape[0]-2, interv):
